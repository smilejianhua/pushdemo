...
var PL = {
	...
	join : function() {
		...
		// Streaming is only supported in Mozilla. E.g. IE does not allow access
		// to responseText on readyState == 3
	...
	_getWebRoot : function() {
		...
		// derive the baseDir value by looking for the script tag that loaded
		// this file
			...
			xmlhttp.onreadystatechange = function() {
				if (xmlhttp.readyState == 4) {
					if (xmlhttp.status == 200) {
						...
						event.put('p_reason',
								'[pushlet] problem retrieving XML data:\n'
										+ xmlhttp.statusText);
						PL._onEvent(event);
					}
				}
	...
	_init : function() {
		...
		/*
		 * Setup Cross-Browser XMLHttpRequest v1.2 Emulate Gecko
		 * 'XMLHttpRequest()' functionality in IE and Opera. Opera requires the
		...
		PL.pushletURL = PL._getWebRoot() + 'eda.srv';
	...
	debug : function(label, value) {
		...
		// Write doc header
		PL.debugWindow.document
				.writeln('<html><head><title>Pushlet Debug Window</title></head><body bgcolor=#DDDDDD>');

		// Write the messages
		for ( var i = 0; i < PL.messagesIndex; i++) {
			PL.debugWindow.document.writeln('<pre>' + i + ': ' + PL.messages[i]
					+ '</pre>');
		}
...
/**
 * Copyright (c) 2012-2015 Wonders Information Co.,Ltd. All Rights Reserved. 5/F
 * 1600 Nanjing RD(W), Shanghai 200040, P.R.C
 * 
 * This software is the confidential and proprietary information of Wonders
 * Group. (Public Service Division / Research & Development Center). You shall
...
function PushletEvent(xml) {
	...
	this.toTable = function() {
		...
		for ( var i in this.arr) {
			res = res + '<tr><td bgColor=white>' + styleDiv + i
					+ '</div></td><td bgColor=white>' + styleDiv + this.arr[i]
					+ '</div></td></tr>';
		}
		res += '</table>'
		return res;
